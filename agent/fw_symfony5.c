...
NR_PHP_WRAPPER(nr_symfony5_exception) {
  ...
  /* Verify that we are using symfony 5, otherwise bail. */
  NR_PHP_WRAPPER_REQUIRE_FRAMEWORK(NR_FW_SYMFONY5);
  ...
  if (0 == nr_php_is_zval_valid_object(event)) {
    nrl_verbosedebug(NRL_TXN,
                     "Symfony 5: KernelEvent::onKernelException() does not "
                     "have an event parameter");
    goto end;
  }

  /* Get the throwable from the event. */
  exception = nr_php_call(event, "getThrowable");
  if (!nr_php_is_zval_valid_object(exception)) {
    nrl_verbosedebug(NRL_TXN,
                     "Symfony 5: getThrowable() returned a non-object");
    goto end;
  }
  ...
  if (NR_SUCCESS
    ...
    nrl_verbosedebug(NRL_TXN, "Symfony 5: unable to record exception");
  }
...
NR_PHP_WRAPPER(nr_symfony5_name_the_wt) {
  zval* event = NULL;
  zval* request = NULL;
   ...
   *
   * RouterListener::onKernelRequest() receives a RequestEvent parameter,
  ...
  if (!nr_php_is_zval_valid_object(event)) {
    nrl_verbosedebug(NRL_TXN,
                     "Symfony 5: RouterListener::onKernelRequest() does not "
                     "have an event parameter");
    NR_PHP_WRAPPER_CALL;
    goto end;
  }
    ...
    if (route_rval) {
      if (NR_SUCCESS
          != nr_symfony_name_the_wt_from_zval(route_rval TSRMLS_CC,
                                               "Symfony 5")) {
        nrl_verbosedebug(
            NRL_TXN, "Symfony 5: Request::get('_route') returned a non-string");
      }
      ...
      if (controller_rval) {
        if (NR_SUCCESS
          ...
          nrl_verbosedebug(
              NRL_TXN,
              "Symfony 5: Request::get('_controller') returned a non-string");
        }
        nr_php_zval_free(&controller_rval);
      } else {
        nrl_verbosedebug(NRL_TXN,
                         "Symfony 5: Neither _controller nor _route is set");
      }
    }
  } else {
    nrl_verbosedebug(NRL_TXN,
                     "Symfony 5: RequestEvent::getRequest() returned a "
                     "non-Request object");
  }
...
void nr_symfony5_enable(TSRMLS_D) {
  /*
   * We set the path to 'unknown' to prevent having to name routing errors.
   */
  nr_txn_set_path("Symfony5", NRPRG(txn), "unknown", NR_PATH_TYPE_ACTION,
  ...
  nr_php_wrap_user_function(NR_PSTR("Symfony\\Component\\HttpKernel\\"
                            ...
                            nr_symfony5_name_the_wt TSRMLS_CC);
  ...
  nr_php_wrap_user_function(
      ...
      nr_symfony5_exception TSRMLS_CC);
}
